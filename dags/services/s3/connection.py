...
def give_up_connection(details) -> None:
    logger.error(
        f"Failed to connect to s3 course of {details}."
        f" Please check connect options or host"
    )
...
def get_s3_hook(s3_connection) -> S3Hook:
    ...
    try:
        ...
        raise ValueError(
            f"The AWS parameters you provided are incorrect: {error}"
        ) from error
